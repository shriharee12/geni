...
function cs_theme_blog_2_columns($atts, $content)
{
	...
	{
		$output .= '<div class="vc_row wpb_row vc_row-fluid' . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '">';
		$i = 0;
		require_once(get_template_directory() . "/shortcodes/class/Post.php");
	...
	{
		$output .= '<div class="vc_row wpb_row vc_row-fluid">' . sprintf(__('No results found for %s', 'carservice'), get_query_var('s')) . '</div>';
	}
	if($cs_pagination)
	{
